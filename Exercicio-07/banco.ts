class Conta_ {
	...
	transferir(contaDestino: Conta_, valor: number): void {
		this.sacar(valor);
...
class Banco_ {
	private _contas: Conta_[] = [];
	
	inserir(conta: Conta_): void {
	...
	consultar(numero: String): Conta_ {
		let contaConsultada!: Conta_;
    ...
    transferir(numeroDebito: string, numeroCredito: string, valor: number){
        let contaCredito: Conta_ = this.consultar(numeroCredito);
        let contaDebito: Conta_ = this.consultar(numeroDebito);
